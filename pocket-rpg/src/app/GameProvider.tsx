...
import { IndexedDbSaveStore } from '../storage/indexed-db';
import { reconstructState } from '../game-core/game-state';
...
interface RuntimeState extends UiGameState {
  ...
  combatMode:'auto'|'manual'; paused:boolean; style:CombatStyle;
...
export function GameProvider({children}:{children:React.ReactNode}){const [state,setState]=useState<RuntimeState>(initial); const [hydrated,setHydrated]=useState(false); const rng=useRef(new SeededRng(1234567)); const saveStore=useRef(new IndexedDbSaveStore());
 useEffect(()=>{saveStore.current.load().then(saved=>{if(saved){const restored=reconstructState(saved,Date.now());setState(s=>({...s,gold:restored.character.gold,world:restored.world,inventory:restored.inventory,market:{...s.market,gold:restored.character.gold,inventory:restored.inventory}}));}}).catch(()=>{}).finally(()=>setHydrated(true));},[]);
 useEffect(()=>{if(!hydrated)return;const id=setTimeout(()=>{saveStore.current.save({character:{level:1,xp:0,gold:state.gold},world:state.world,inventory:state.inventory}).catch(()=>{});},300);return()=>clearTimeout(id);},[hydrated,state.gold,state.world,state.inventory]);
